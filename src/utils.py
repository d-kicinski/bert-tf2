    ...
    def load_google_bert(model: BertModel,
        ...
        for var_name, _ in var_names:
            ...
            if w_id is not None and qkv is None:
                logging.info(var_name, ' -> ', model.weights[w_id].name)
                if w_id in [0, 1, 2]:  # embeddings
                    print(var_name, ' -> ', model.weights[w_id].name)
                    if unsqueeze:
                        weights[w_id][:max_sequence_length, :] = checkpoint.get_tensor(var_name)[None,
                                                                 :max_sequence_length, :]
                    else:
                        weights[w_id][:max_sequence_length, :] = checkpoint.get_tensor(var_name)[:max_sequence_length,
                                                                 :]
                ...
                # saved = checkpoint.get_tensor(var_name)  # vocab_size, emb_size
            ...
            #         weights[w_id][p * dim_size:(p + 1) * dim_size] = check_point.get_tensor(var_name)
                ...
                logging.info('not mapped: ', var_name)
